...
4. Read `Logs/AlienCrusherResourceProductionBacklog.md` if the next safe lane is resource planning, starting with `## Recommended Production Batch Order`, then use `## Production Batch Focus` for full batch details.
...
- Prefer complete production batches from `## Recommended Production Batch Order` over isolated asset tasks, so unattended work improves a full readable gameplay beat.
