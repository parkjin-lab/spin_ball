...
- This pass adds inspectable drafts at `Assets/Art/VFX/Combo/` plus Resources copies. `ComboOverdrivePulseVfx` `Resources.Load`s them (editor `AssetDatabase` fallback), instantiates a copy, then applies the existing lime/gold and orange/flame colors. Combo and Overdrive now use separate particle caches so the two drafts stay distinct. Combo thresholds, overdrive duration/damage, and HUD layout are unchanged. No Gap* density and no HUD/lobby residual pulse.
- How to see it: Play Mode Stage 1 — land a CRUSH RUSH for lime-gold `VFX_Combo_Rise_Pulse` upward ticks, then trigger OVERDRIVE for an orange `VFX_Overdrive_Pulse` speed ring plus flame chevrons. They should not share one look, and they should not look like route mint/magenta or failure umber. Project window: `Assets/Art/VFX/Combo/`.

## 2026-08-27 Route Payoff VFX Draft Materials

- Checked first: Outgame Progression UI (`UI_DP_GainBurst`, `UI_FormCard_StateSet`, `UI_MetaNode_*`, `Badge_FormReady`, `Badge_MetaReady`, `Banner_StageUnlocked`, `Toast_ProgressionSaved`, `SFX_Progression_*`) is already on disk and wired. Audio A–C plus leftover Audio D clips (`SFX_Failure_Warning`, `SFX_Failure_Boss`) are already on disk. Street Props A–D are runtime silhouette kits (`PROP_*` child ids) with no named `MAT_PROP_*` slots — did not invent kit materials. HUD/lobby residual pulses (`VFX_FormEquip_Confirm` through `VFX_NextAction_Residual`) were not drafted.
- The next unused COMPLETE on-disk gap was Route Payoff B–I named VFX: runtime already played `VFX_RouteCluster_Marker`, `VFX_ForwardSmash_Confirm`, `VFX_RouteHold_Success`, `VFX_RouteOpen_Trail`, `VFX_LaneBreak_Residual`, `VFX_RouteChase_Pulse`, `VFX_RouteHold_Warning`, and `VFX_RouteBonus_Success` from anonymous Unlit copies. Docs already pointed those files at `Assets/Art/VFX/Route/`.
- This pass adds those inspectable drafts plus Resources copies. `RoutePayoffVfxDrafts` `Resources.Load`s them (editor `AssetDatabase` fallback). Existing route VFX classes instantiate a copy, then apply the current mint / gold-cyan / magenta / ivory / cobalt / rose / amethyst recipes. Route timing, payoff counts, target placement, and marker tints are unchanged. No Gap* density and no HUD/lobby residual pulse.
- How to see it: Play Mode Stage 1 — LANE BREAK leaves a tiny ivory `VFX_LaneBreak_Residual`; ROUTE OPEN races magenta `VFX_RouteOpen_Trail`; HOLD closing is rose `VFX_RouteHold_Warning`; HOLD clear is gold-cyan `VFX_RouteHold_Success`; ROUTE BONUS is an amethyst `VFX_RouteBonus_Success` bloom plus mint-slate `VFX_RouteCluster_Marker` ring; chase uses cobalt `VFX_RouteChase_Pulse`; FORWARD SMASH is a mint-white `VFX_ForwardSmash_Confirm` star. Orchid Target_A/B and HOLD pips stay louder. Project window: `Assets/Art/VFX/Route/`.

## 2026-08-27 Failure Beat VFX Draft Materials

- UI Icons A–D named files were already on disk and wired: `Icon_DP`, `Icon_Stage`, `Icon_NextStep`, `Icon_Route`, `Icon_BreakWindow`, `Icon_Shield`, `Icon_WeakPoint`, `Icon_Boss`, `Icon_Overdrive`, `Icon_Panic`, `Icon_Seismic`, `Icon_Retail`, `Icon_Traffic` at `Assets/Resources/UI/Icons/`, plus `Badge_Result_Clear`, `Badge_Result_Failure`, `Badge_Locked`, and `Badge_Recommended` at `Assets/Resources/UI/Badges/`. Audio A–C clip files were also already on disk. The next unused COMPLETE on-disk gap was leftover `[Audio] D. Failure beat visuals`: runtime already played `VFX_Failure_Ordinary` / `VFX_Failure_Boss`, but those were anonymous Unlit copies with no inspectable files.
- This pass adds inspectable drafts at `Assets/Art/VFX/Failure/` plus Resources copies. `FailureBeatVfx` `Resources.Load`s them (editor `AssetDatabase` fallback), instantiates a copy, then applies the existing umber/tan and slag/ash colors so PulseFlash still fades. Fail timing, HP, and boss pressure are unchanged. No Gap* density and no HUD/lobby residual pulse.
- How to see it: Play Mode HUD already shows money (`Icon_DP`), stage, next action, route, shield, weak point, boss, and result clear/fail badges without reading long text. Fail a Stage 1 run — dry umber `VFX_Failure_Ordinary` down-break, not smash ticks. `F7` Stage 4, fail during the boss phase — heavier steel `VFX_Failure_Boss` collapse, not the Sentinel-down cascade. Project window: `Assets/Art/VFX/Failure/`.
...
- This pass adds those drafts at `Assets/Art/Materials/Boss/` and `Assets/Art/VFX/Boss/` plus Resources copies. `BossIdentityMaterials` `Resources.Load`s the three armor/pylon/core mats (editor `AssetDatabase` fallback; Destruction B `MAT_Shielded_Pylon` / `MAT_Exposed_Core` are fallbacks only). `ApplyNamedBossIdentityKit` assigns armor to Sentinel body parts and the cyan pylon mat to gate posts/pane/caps. Combat state prefers the Boss Identity pylon/core mats on live kits. `BossClimaxFeedbackVfx` loads the three VFX drafts onto the existing warning ring, core-expose burst, and defeat cascade slots. Boss HP, timing, exposure, drone counts, and pressure are unchanged. No Gap* density and no HUD/lobby residual pulse.
- Stage 4 assignment notes (no HUD required): the tall biped `BOSS_Sentinel_Body_Kit` reads steel-blue `MAT_Boss_Sentinel_Armor`, not plaster/concrete. Live `BOSS_Shield_Pylon_Kit` gates read cyan `MAT_Boss_Shield_Pylon` on pane/caps/`ShieldBarrier`. Phase-2 `BOSS_Phase2_Drone_Kit` hover bodies stay ember `(1.00, 0.32, 0.16)` — smaller and warmer than the Sentinel, not cyan. When `bossCoreExposureActive`, `ChestCore` / `VisorAccent` plus the core ring flip to hot `MAT_Boss_Core_Exposed`. Pressure uses rust-amber `VFX_Boss_Warning_Ring`; shield-drop / CORE EXPOSED uses hot `VFX_Boss_Core_Expose_Burst`; Sentinel down uses steel-ice `VFX_Boss_Defeat_Cascade`. Steel / cyan / ember / orange stay four different reads.
- How to see it: Play Mode `F7` Stage 4. Count the tall steel Sentinel, the cyan gate pylons, and (in phase 2) the small ember hover drones without reading HUD text. When shields drop, the chest core goes hot orange and a short vertical `VFX_Boss_Core_Expose_Burst` fires — not another warning ring. A pressure pulse is a rust-amber ground ring; defeat is steel-white shards. Project window: `Assets/Art/Materials/Boss/` and `Assets/Art/VFX/Boss/`.

## 2026-08-27 Form Identity D Draft Materials

- Form icons and silhouette kits were already on disk and wired (`Icon_Form_Sphere` / `Ram` / `Saucer` / `Spike` / `Crusher`, plus skill icons including `Icon_Skill_SpherePulse`, and runtime `FORM_*_Body_Kit`s). The unused COMPLETE gap was Form Identity D material families: no `MAT_Form_*` files existed, so ball pieces still used anonymous `M_Runtime_*` Unlit copies.
- This pass adds inspectable drafts `MAT_Form_Sphere` (cool green), `MAT_Form_Ram` (amber plate), `MAT_Form_Saucer` (cyan rim), `MAT_Form_Spike` (acid tip), and `MAT_Form_Crusher` (steel + blue seam emission) at `Assets/Art/Materials/Forms/` plus Resources copies. `FormIdentityMaterials.Create` `Resources.Load`s the matching family (editor `AssetDatabase` fallback), instantiates it, then applies the existing piece colors so pulse flashes still work. Form stats, unlock costs, and skill numbers are unchanged. No Gap* density and no HUD/lobby residual pulse.
- How to see it: Play Mode lobby — the five form cards already show distinct icons (green belt-sphere, amber wedge, cyan disc, acid spikes, steel-blue mass) without reading names. Equip Sphere / Ram / Saucer / Spike / Crusher and start a run: the ball uses the same color families (green belt, amber horns, cyan rim, acid needles, blue seams). Project window: `Assets/Art/Materials/Forms/`.

## 2026-08-27 Destruction Readability C Draft Assets

- `[Destruction Readability] C. Break feedback package` was already wired in C# (`DestructionBreakFeedbackVfx.cs`, smash-weight slots from `9920b18`). This pass adds inspectable draft materials so the four named VFX are not one shared runtime particle mat: `VFX_Debris_Light`, `VFX_Debris_Heavy`, `VFX_Smoke_Damage`, `VFX_WeakPoint_Hit` at `Assets/Art/VFX/Destruction/` plus Resources copies at `Assets/Resources/VFX/Destruction/`. Runtime `Resources.Load`s those drafts onto the existing ParticleSystem / flash slots (editor `AssetDatabase` fallback) and only creates Particles Unlit copies if the assets are missing. No `.prefab` / `.unity` YAML. HP, break thresholds, scores, timing, and smash-weight mappings are unchanged. No Gap* density and no HUD/lobby pulse.
- Recipes stay the existing burst colors so the four reads never share a look: light plaster chips `(0.84, 0.76, 0.62)`; heavy charcoal `(0.22, 0.20, 0.18)` plus ember emission `(0.72, 0.32, 0.10)`; near-break smoke grey `(0.20, 0.20, 0.22, 0.55)`; weak-point gold `(1.00, 0.84, 0.16)` plus hot emission. Particle counts, cone size, and duration stay the existing runtime bursts so Target_A/B and HOLD pips stay readable.
- Stage 1 / Stage 5 assignment notes (no HUD required): Stage 1 `GapLamp_*` / trees / benches stay `SmashBreakWeight.Light` → plaster `VFX_Debris_Light` chips, no smoke. Mid `Block_*` destroy uses `VFX_Debris_Heavy` at the existing 0.58 intensity. Stage 5 `YardCraneMast` stays Heavy → `VFX_Debris_Heavy` at 0.9 plus `VFX_Smoke_Damage` on the host while durability is mid-low. Elite / core crits fire gold-or-orange `VFX_WeakPoint_Hit` (gold pip vs hotter core) instead of ordinary chips.
- How to see it: Play Mode Stage 1 — smash a `GapLamp_*` then a `Block_*`. Lamp chips pale plaster; the block bursts darker charcoal/ember. `F7` Stage 5, smash `YardCraneMast` — heavier charcoal burst plus grey near-break smoke, not lamp chips. A weak-point crit is a tight gold flash, not debris. HUD text is not required. Project window: `Assets/Art/VFX/Destruction/` shows the four drafts.

## 2026-08-27 Destruction Readability B Draft Assets

- `[Destruction Readability] B. Combat state materials` was already wired in C# (`DummyDestructibleBlock.CombatStateMaterials.cs`). This pass adds inspectable draft `.mat` files so the four states are not runtime-only: `MAT_Damage_CrackOverlay`, `MAT_WeakPoint_Glow`, `MAT_Shielded_Pylon`, `MAT_Exposed_Core` at `Assets/Art/Materials/Destruction/` plus Resources copies at `Assets/Resources/Materials/Destruction/`. Runtime now `Resources.Load`s those drafts (editor `AssetDatabase` fallback) and only creates Unlit copies if the assets are missing. Assignment, HP, break thresholds, shield counts, scores, and timing are unchanged. No Gap* density and no HUD/lobby pulse.
- Recipes stay the existing combat colors so the four states never share a look: crack overlay dark ember `(0.07, 0.04, 0.03)` / emission `(0.20, 0.05, 0.02)` metallic 0.02 / smoothness 0.08; weak-point gold pip `(1.00, 0.84, 0.14)` / emission `(0.72, 0.42, 0.04)` 0.08 / 0.42 plus `_WeakPointHalo`; shielded pylon cyan `(0.16, 0.78, 0.94)` / emission `(0.06, 0.38, 0.58)` 0.22 / 0.55; exposed core hot orange `(1.00, 0.40, 0.08)` / emission `(0.95, 0.32, 0.04)` 0.18 / 0.62.
- Stage 4 assignment notes (no HUD required): live `BOSS_Shield_Pylon_Kit` panes / caps / `ShieldBarrier` use cyan `MAT_Shielded_Pylon` and hide the gold halo. When `bossCoreExposureActive`, Sentinel `ChestCore` / `VisorAccent` plus core ring / telegraph use hot `MAT_Exposed_Core`. Ordinary elite weak points (not a live pylon, core closed) keep gold `MAT_WeakPoint_Glow` plus the halo ring. Damaged city blocks grow dark `MAT_Damage_CrackOverlay` split lines after about 22% durability loss. Gold / cyan / orange / dark-ember never share one material.
- How to see it: Play Mode `F7` Stage 4. A live shield pylon reads cyan barrier, not gold and not orange. When shields drop, the Sentinel chest/core flips to hot orange-white. Elite weak-point pips stay gold with a halo. Damaged `Block_*` grow dark crack lines. HUD text is not required. Project window: `Assets/Art/Materials/Destruction/` now also shows the four combat-state drafts next to the building-tier set.

## 2026-08-27 Destruction Readability A Draft Assets

- `[Destruction Readability] A. Building tier materials` was already wired in C# (`DummyFlowController.BuildingTierMaterials.cs`). This pass adds inspectable draft `.mat` files so the four tiers are not runtime-only: `MAT_Building_Small`, `MAT_Building_Mid`, `MAT_Building_Large`, `MAT_Boss_Structure` at `Assets/Art/Materials/Destruction/` plus Resources copies at `Assets/Resources/Materials/Destruction/`. Runtime now `Resources.Load`s those drafts (editor `AssetDatabase` fallback) and only creates Unlit copies if the assets are missing. Assignment, HP, break thresholds, scores, route timing, targets, map size, boss pressure, and DP are unchanged. No extra Gap* density.
- Recipes stay the existing healthy/hit colors: small pale plaster `(0.86, 0.78, 0.64)` / hit `(0.96, 0.70, 0.46)`; mid city concrete `(0.46, 0.42, 0.38)` / hit `(0.78, 0.48, 0.28)`; large charcoal `(0.16, 0.17, 0.20)` / hit `(0.92, 0.36, 0.14)`; boss steel-blue `(0.20, 0.30, 0.46)` / hit `(0.58, 0.80, 1.00)`. Sheen: small metallic 0 / smoothness 0.06; mid 0.04 / 0.16; large 0.14 / 0.28 plus faint emission; boss 0.32 / 0.44 plus cool emission.
- Stage 1 assignment notes (no HUD required): `StarterLotProp_*` / `StarterLaneProp_*` are small plaster by name. `StarterLaneBlock_*` and `StarterClusterBlock_*` stay small (height about 0.54-1.04, HP 1-4, under the existing large gates of HP 5 / height 3.6 / volume 55). Opening `Block_*` that pass `IsSmallBuildingTier()` (height <= 1.8 and footprint <= 2.8, not large) are plaster. Other non-large `Block_*` are mid concrete. Any `Block_*` that trips `IsLargeBuildingTarget` is charcoal. Stage 1 has no Sentinel / gate / pylon hosts, so no steel-blue armor. Street `Gap*` kits keep their own Street Props materials.
- Stage 4 assignment notes (no HUD required): city blocks use the same small / mid / large rules. Steel-blue `MAT_Boss_Structure` goes to `IsStageBoss` / `BossSentinel`, names starting `Sentinel` (`SentinelPylonForeshadow_*`), names containing `ShieldPylon` or `GateBlock` (`SentinelGateBlock`), and hosts with live `BOSS_Sentinel_Body_Kit` / `BOSS_Shield_Pylon_Kit` / `BOSS_Phase2_Drone_Kit`. Foreshadow pylons and the gate stay boss-colored even when their height/HP would otherwise read small.
- How to see it: Play Mode Stage 1 — starter lot/lane props and low opening blocks are pale plaster, mid-route fillers are warmer concrete, and any tall/high-HP `Block_*` is charcoal; no steel-blue armor. `F7` Stage 4 — same city tiers plus steel-blue Sentinel body, `SentinelGateBlock`, foreshadow pylons, and live shield pylons. HUD text is not required. Project window: `Assets/Art/Materials/Destruction/` shows the four drafts.
...
- 8-10 thicken the starter lane (22-32 kits, inner+outer curbs) so first-contact smashables sit in the F10 opening window. Landmark / Sentinel keep-out still skip CivicCore, TransitHub, boss arena, pylons, and weak points. Target_A/B stay 3.4m clear; HOLD trail stays on the center guide.
- Opening-count now also includes smash-path street props in that window (moving `Car_RT_*` stay out), so the 2026-08-24 `starter lane sparse` warning tracks real first-contact smashables.
- How to see it: Play Mode Stage 8, then `F7` to Stage 10. Spawn/starter curbs should already show `GapLamp_Open_*` / `GapCar_Open_*` before rolling far. F10 layout debug should no longer warn `starter lane sparse` on 08/09/10, or the counts should rise well above 20/16/13. CivicCore / Sentinel stay uncovered.
...
- This is baked into `BuildFallbackMap` / scaffolder opening curbs, not the late `EnsureOpeningDestructionDensity` patch. Lane-break thresholds, timers, payoff counts, and map size are unchanged.
- How to see it: Play Mode, start Stage 1 — spawn-lane curbs already show `GapLamp_Open_*` / `GapCar_Open_*` before moving far. F7 to Stage 2 and Stage 3: same first-contact curb smashables. Console should not need a later "Opening density was low" fill to make the opener smashable.
...
- Form Identity D material families now have inspectable drafts: `MAT_Form_Sphere` / `Ram` / `Saucer` / `Spike` / `Crusher` at `Assets/Art/Materials/Forms/` plus Resources copies. Runtime kits and lobby/skill icons were already shipped; this pass only added the missing on-disk material families. Form stats and unlocks are unchanged.
- Shipped production batch `[Destruction Readability] A. Building tier materials`: draft `.mat` files `MAT_Building_Small` (pale plaster), `MAT_Building_Mid` (city concrete), `MAT_Building_Large` (charcoal mass), and `MAT_Boss_Structure` (cool steel-blue armor) live at `Assets/Art/Materials/Destruction/` plus Resources copies. Runtime loads those drafts, then assigns from existing height/HP/role/name checks. Stage 1 reads cheap plaster + concrete; Stage 4 adds dark durables plus Sentinel/gate/pylon boss armor. HP, break thresholds, and spawn counts are unchanged.
- Shipped production batch `[Destruction Readability] B. Combat state materials`: draft `.mat` files `MAT_Damage_CrackOverlay`, `MAT_WeakPoint_Glow`, `MAT_Shielded_Pylon`, and `MAT_Exposed_Core` live at `Assets/Art/Materials/Destruction/` plus Resources copies. Runtime loads those drafts, then assigns from existing pylon / exposure / weak-point / damage checks. Stage 1 smash shows gold weak-point pips plus crack overlays; Stage 4 shows cyan protected pylons vs a hot exposed core. HP, shield counts, and timing are unchanged.
- Shipped production batch `[Destruction Readability] C. Break feedback package`: draft materials `VFX_Debris_Light`, `VFX_Debris_Heavy`, `VFX_Smoke_Damage`, and `VFX_WeakPoint_Hit` live at `Assets/Art/VFX/Destruction/` plus Resources copies. Runtime loads those drafts onto the existing light/mid/heavy / smoke / weak-point slots. Bursts stay local so Target_A/B and HOLD pips remain readable. HP, spawn counts, and smash-weight mappings are unchanged.
...
- Shipped combo/Overdrive visual pulses: inspectable drafts `VFX_Combo_Rise_Pulse` (lime-gold upward ticks on CRUSH RUSH) and `VFX_Overdrive_Pulse` (orange speed ring plus flame chevrons on OVERDRIVE) now live at `Assets/Art/VFX/Combo/` plus Resources copies. Reuses Audio C `comboRiseClip` already hooked in `PlayComboRushFeedback`. Combo thresholds, overdrive duration/damage, and HUD layout are unchanged. Form Identity D remains complete.
...
- Shipped leftover `[Route Payoff] G. Forward chase pulse`: `VFX_RouteChase_Pulse` is cobalt wedges plus an ice diamond ping from the opened ROUTE BONUS cluster toward the next smash target. Reuses the existing ROUTE BONUS / combo-rush audio. OPEN trail, HOLD pulse, smash confirm, combo pulses, climax VFX, ambient bands, and lobby confirm audio were not restyled.
- Shipped leftover `[Route Payoff] H. HOLD warning pulse`: `VFX_RouteHold_Warning` is rose inward ticks plus a blush ping at the beacon when ROUTE HOLD CLOSING fires. Reuses Audio A `routeHoldWarningClip` already hooked in `PlayHudWarningFeedback`. Chase pulse, OPEN trail, HOLD success, smash confirm, combo pulses, climax VFX, ambient bands, and lobby confirm audio were not restyled.
- Shipped leftover `[Route Payoff] I. ROUTE BONUS success pulse`: `VFX_RouteBonus_Success` is a small amethyst bloom plus upward petals at the opened cluster when ROUTE BONUS fires. Reuses Audio A `routeBonusClip` already hooked in `PlayTotalDestructionFeedback`. Chase wedges, smash star, HOLD lock, OPEN dashes, LANE BREAK residual, combo pulses, climax VFX, ambient bands, and lobby confirm audio were not restyled.
- Route Payoff B–I named VFX now also have inspectable drafts at `Assets/Art/VFX/Route/` plus Resources copies. Runtime `RoutePayoffVfxDrafts` loads those drafts onto the existing cluster / smash / HOLD / OPEN / residual / chase / warning / bonus slots. Combo/Overdrive pulses now have their own drafts at `Assets/Art/VFX/Combo/`. HUD/lobby residual pulses were not drafted.
- Shipped leftover `[UI Icons] E. Failure bucket badges`: result outcome plate swaps `Badge_Fail_Opening` / `Badge_Fail_Hold` / `Badge_Fail_Drift` / `Badge_Fail_Push` / `Badge_Fail_Boss` from the last-run fail bucket. Generic rust `Badge_Result_Failure` stays the leftover fallback. Route pulses A–I, smash confirm, climax VFX, ambient bands, and lobby confirm audio were not restyled.
...
- Shipped leftover `[Outgame Progression] E. Form equip confirm pulse`: `VFX_FormEquip_Confirm` is a short champagne ring on the equipped lobby form card. Reuses `SFX_Progression_Confirm` already hooked on equip. Form-card color states, result badges, and in-run smash/route VFX were not restyled.
- Shipped leftover `[Outgame Progression] F. Next-run spend change pulse`: `VFX_SpendChange_Ready` is a short jade ready plate on the HUD at stage start after a lobby form equip or meta purchase, naming what changed. Confirm audio stays on the lobby spend hooks. Form-equip champagne ring, result badges, route pulses, smash confirm, climax VFX, and ambient bands were not restyled.
- Shipped leftover `[Outgame Progression] G. Stage select confirm pulse`: `VFX_StageSelect_Confirm` is a short ice-slate bracket pulse on the lobby stage readout when Stage prev/next changes the selected stage. Confirm audio is not reused. Form-equip champagne ring, spend-change plate, result badges, unlock banners, route pulses, smash confirm, climax VFX, and ambient bands were not restyled.
- Shipped leftover `[Outgame Progression] H. Meta purchase confirm pulse`: `VFX_MetaUpgrade_Confirm` is a short copper diamond pulse on the purchased Size / Impact / DP node. Reuses `SFX_Progression_Confirm` already hooked on spend. Form-equip champagne ring, stage-select brackets, spend-change plate, result badges, route pulses, smash confirm, climax VFX, and ambient bands were not restyled.
- Shipped leftover `[Audio] D. Failure beat visuals`: inspectable drafts `VFX_Failure_Ordinary` and `VFX_Failure_Boss` now live at `Assets/Art/VFX/Failure/` plus Resources copies. Ordinary defeat is a dry umber down-break; boss-phase defeat is a heavier steel collapse. Reuses Audio A failure clips already hooked in `PlayFailureBeatFeedback`. Smash/combo ticks, boss-down cascade, route pulses, lobby confirm VFX, and ambient bands were not restyled.
- Shipped leftover `[Outgame Progression] I. DP gain residual afterglow`: `VFX_DP_Gain_Residual` is a short aqua halo beside the existing `UI_DP_GainBurst` on result/lobby earn. The burst sprite was not restyled. Copper meta diamond, champagne form-equip ring, ice-slate stage-select brackets, route pulses, smash confirm, climax VFX, and ambient bands were not restyled.
- Shipped leftover `[Outgame Progression] J. Result next-action ready pulse`: `VFX_NextAction_Ready` is a short lilac caret on the one form-ready, meta-ready, or recommended result badge when result opens. Aqua DP residual, copper meta diamond, champagne form-equip ring, ice-slate stage-select brackets, jade spend-change plate, route pulses, smash confirm, climax VFX, and ambient bands were not restyled.
- Shipped leftover `[Outgame Progression] K. Stage unlock banner pulse`: `VFX_StageUnlock_Ready` is a short honey-ivory residual afterglow beside the existing `Banner_StageUnlocked` when a first-time stage clear unlocks the next stage. The banner sprite was not restyled. Lilac next-action caret, ice-slate stage-select brackets, aqua DP residual, copper meta diamond, champagne form-equip ring, jade spend-change plate, route pulses, smash confirm, climax VFX, and ambient bands were not restyled.
- Shipped leftover `[Outgame Progression] L. Form unlock confirm pulse`: `VFX_FormUnlock_Confirm` is a short periwinkle petal burst on a newly unlocked lobby form card. Champagne form-equip ring stays on re-equip only. Honey-ivory stage-unlock residual, lilac next-action caret, aqua DP residual, copper meta diamond, ice-slate stage-select brackets, jade spend-change plate, route pulses, smash confirm, climax VFX, and ambient bands were not restyled.
- Shipped leftover `[Outgame Progression] M. Result next-action residual afterglow`: `VFX_NextAction_Residual` is a short pale-orchid halo beside the existing `VFX_NextAction_Ready` caret when result opens with a next action. The caret was not restyled. Periwinkle form-unlock burst, champagne form-equip ring, honey-ivory stage-unlock residual, aqua DP residual, route pulses, smash confirm, climax VFX, and ambient bands were not restyled.
