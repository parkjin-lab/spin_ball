...
- Latest static audit refresh: 2026-06-10 (`Tools/RunPlaytestReadinessPrep.ps1` now refreshes static audits, stage checklist, autonomous work backlog, optional production checklists, telemetry summary readiness artifact, and Evidence Gate report-only readiness)
...
- While waiting, unattended agents should use `Logs/AlienCrusherAutonomousWorkBacklog.md` and `Docs/AUTOMATION_RUNBOOK.md` for safe non-tuning work.
...
4. Run the progression save smoke pass from the generated stage checklist.
5. Run one Unity `F10` sweep and capture Stage 1 / 4 / 7 notes in `Docs/AlienCrusherStagePlaytestNotes.md`
6. Re-run `powershell -ExecutionPolicy Bypass -File Tools/RunPlaytestReadinessPrep.ps1`, then compare the rhythm snapshot against the checklist notes
