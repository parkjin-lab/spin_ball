...
- Shipped production batch `[Street Props] B. Light roadside rhythm props`: named runtime kits `PROP_StreetLamp`, `PROP_TrafficLight`, `PROP_RoadsideTree`, and `PROP_Bench` replace the box/pole/blob roadside meshes. Stage 1-3 streets get thin lamps and signal heads, gappy trees, and low starter-lane benches. Spawn counts and HP are unchanged. Street Props C (kiosk/vending/transformer/barrel) was not started.
