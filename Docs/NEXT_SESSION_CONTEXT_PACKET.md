...
- Shipped `[District Palette] D. Ambient stage bands`: named set `PAL_Ambient_StageBands` drives `RenderSettings.ambientLight` per stage so opener / Sentinel / skyline no longer share one fill. Fog stays off. District palettes A/B/C and route tints were not restyled. Form Identity D remains complete. Street Props D and Boss Identity B leftover expose burst were not started.
