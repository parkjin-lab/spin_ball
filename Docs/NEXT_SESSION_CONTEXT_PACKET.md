...
- `Assets/Scripts/Runtime/Systems/DummyFlowController.ProgressionCore.cs`
  - LANE BREAK now starts `LANE BREAK -> ROUTE OPEN`, sets the route open beat timer, and resets that runtime timer at stage setup. ROUTE BONUS now spawns a district-flavored Forward Smash reward cluster around the highlighted target.
