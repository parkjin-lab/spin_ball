...
- Shipped leftover `[Audio] D. Failure beat visuals`: `VFX_Failure_Ordinary` is a dry umber down-break on ordinary defeat, and `VFX_Failure_Boss` is a heavier steel collapse on boss-phase defeat. Reuses Audio A failure clips already hooked in `PlayFailureBeatFeedback`. Smash/combo ticks, boss-down cascade, route pulses, lobby confirm VFX, and ambient bands were not restyled.
