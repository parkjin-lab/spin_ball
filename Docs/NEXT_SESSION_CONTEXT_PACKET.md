...
- Shipped production batch `[Outgame Progression] A. DP economy signal`: `UI_DP_GainBurst` tints earn / spend / insufficient on result and lobby DP labels, and `SFX_Progression_Locked` plays on failed form/meta purchases. `Icon_DP` was not restyled. HUD copy and DP/cost numbers are unchanged. Outgame B/C/D and Boss Identity B were not started.
