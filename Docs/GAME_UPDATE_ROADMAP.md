...
- Boss climax feedback now uses named runtime VFX `VFX_Boss_Warning_Ring` and `VFX_Boss_Defeat_Cascade` so inbound/pressure-pulse warning and Sentinel-down release have distinct visual weight from the existing Audio C clips `SFX_Boss_Warning`, `SFX_Boss_Break`, and `SFX_Boss_Down`. Shield/exposed/broken still use Destruction B cyan / hot orange / dark cracks. Boss timing, HP, and pulse intervals are unchanged.
