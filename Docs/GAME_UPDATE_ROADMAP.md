...
- FeedbackSystem now exposes assignable audio hooks for hit weight, destruction size, combo rise, route open/hold/bonus beats, boss warnings/break/down, and level-up moments. Route/failure drafts and hit/break weight drafts (`SFX_Hit_Light`, `SFX_Hit_Medium`, `SFX_Hit_Heavy`, `SFX_Break_Small`, `SFX_Break_LargeCollapse`) load at runtime if Inspector slots are empty. Remaining boss/level-up/combo slots stay silent-safe until assigned.
