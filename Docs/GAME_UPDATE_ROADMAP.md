...
- Leftover `[Audio] D. Failure beat visuals` now ships `VFX_Failure_Ordinary` and `VFX_Failure_Boss`: ordinary defeat drops a dry umber break, boss-phase defeat drops a heavier steel collapse. Distinct from smash/combo ticks, boss-down cascade, route pulses, and lobby confirm VFX. Failure audio stays on the existing `PlayFailureBeatFeedback` hook. Timing and HP are unchanged.
