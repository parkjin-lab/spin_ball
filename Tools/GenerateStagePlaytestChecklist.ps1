...
$lines.Add("- Progression save safety audit: $(Format-ResultLine -Path (Join-Path $projectRoot "Logs\AlienCrusherProgressionSaveSafetyStaticAudit.log"))")
$lines.Add("")
$lines.Add("## Controls")
$lines.Add("")
...
$lines.Add('- Playtest summary command: `powershell -ExecutionPolicy Bypass -File Tools/GeneratePlaytestTelemetrySummary.ps1` (includes current tuning snapshot, rhythm snapshot, `Tune Next`, sweep, stage trend, tuning candidate, first-pass experiment, and failure bucket summaries when `F10` is used)')
$lines.Add("")
$lines.Add("## First F10 Sweep Run Sheet")
$lines.Add("")
$lines.Add("- [ ] Start from Stage 1 with the map overlay available.")
$lines.Add('- [ ] Press `F10` once and do not tune during the sweep.')
$lines.Add('- [ ] Confirm `Logs/AlienCrusherPlaytestTelemetry.log` exists after the sweep.')
$lines.Add("- [ ] Confirm the regenerated summary was created after the telemetry log.")
$lines.Add("- [ ] Expected minimum telemetry markers:")
$lines.Add('  - `SWEEP_START`')
$lines.Add('  - `STAGE_START` for Stage 01, Stage 04, Stage 07')
$lines.Add('  - `STAGE_END` for Stage 01, Stage 04, Stage 07')
$lines.Add('  - `SWEEP_END`')
$lines.Add("- [ ] If the log is missing or markers are absent, fix telemetry/sweep wiring before any balance tuning.")
$lines.Add("")
...
$lines.Add("- [ ] Exit and re-enter play mode, then confirm DP, selected form, unlocked stage, and meta upgrade state persisted.")
$lines.Add("- [ ] If a test save was edited or restored, confirm impossible meta/stage values are repaired and do not reappear after reload.")
$lines.Add("- [ ] Treat save/load failures as P0 validation blockers before rhythm tuning.")
$lines.Add("")
...
$lines.Add('- [ ] Current values copied from `Tune Next`:')
