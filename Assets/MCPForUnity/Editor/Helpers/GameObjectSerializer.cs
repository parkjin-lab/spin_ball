    ...
    public static class GameObjectSerializer
    {
        ...
        public static object GetGameObjectData(GameObject go)
        {
            ...
            return new
            {
                ...
                parentInstanceID = go.transform.parent?.gameObject.ToSerializedId() ?? 0, // 0 if no parent
        ...
        {
            // --- Add Early Logging --- 
            // McpLog.Info($"[GetComponentData] Starting for component: {c?.GetType()?.FullName ?? "null"} (ID: {c?.ToSerializedId() ?? 0})");
            // --- End Early Logging ---
            ...
            if (componentType == typeof(Transform))
            {
                ...
                // McpLog.Info($"[GetComponentData] Manually serializing Transform (ID: {tr.ToSerializedId()})");
                return new Dictionary<string, object>
                {
                    { "typeName", componentType.FullName },
                    { "instanceID", tr.ToSerializedId() },
                    ...
                    { "parentInstanceID", tr.parent?.gameObject.ToSerializedId() ?? 0 },
                    { "rootInstanceID", tr.root?.gameObject.ToSerializedId() ?? 0 },
                    { "childCount", tr.childCount },
                    // Include standard Object/Component properties
                    { "name", tr.name },
                    { "tag", tr.tag },
                    { "gameObjectInstanceID", tr.gameObject?.ToSerializedId() ?? 0 }
                };
            }
            ...
            if (componentType == typeof(Camera))
            {
                ...
                var safeProperties = new Dictionary<string, Func<object>>
                {
                    ...
                    { "name", () => cam.name },
                    { "tag", () => cam.tag },
                    { "gameObject", () => new { name = cam.gameObject.name, instanceID = cam.gameObject.ToSerializedId() } }
                };
